...
from util.util import lowpass_fir
...
def calc_behaviour(sn=None, session=None):
    ch_idx = np.array(dn.diffCols)

    dat = pd.read_csv(os.path.join(pth.baseDir, 'behavioural', f'day{session}', f'efc4_{sn}.dat'),
                      sep='\t')
    ...
    prefix = f'efc4_{sn}'
    ext = '.mov'
    path = os.path.join(pth.baseDir, 'behavioural', f'day{session}')
    block_files = [f for f in os.listdir(path) if f.startswith(prefix) and f.endswith(ext)]
    blocks = [int(f.split("_")[-1].split('.')[0]) for f in block_files]
    ...
    for bl in blocks:
        ...
        for ntrial in TN:
            ...
            # get force
            force_tmp = mov[mov[:, 0] == ntrial][:, ch_idx] * dn.fGain

            # get tinfo
            dat_row = dat_tmp[dat_tmp.TN == ntrial].reset_index()

            # calc absolute derivative
            force_filt = lowpass_fir(force_tmp, n_ord=4, cutoff=10, fsample=dn.fsample['force'], axis=0)
            force_der1 = np.gradient(force_filt, 1 / dn.fsample['force'], axis=0)
            force_der1_abs = np.abs(force_der1)
            force_der1_peak = force_der1_abs.max(axis=0)
            force_der1_t2p_samples = np.argmax(force_der1_abs, axis=0)
            force_der1_avg = force_der1_abs.mean(axis=0)

            chordID = dat_row['chordID'][0]
            channels = [i for i, c in enumerate(str(chordID)) if c in ('1', '2')]

            et_samples = find_sustained_threshold_crossing(np.abs(force_filt), channels, dn.ftarget, fsample=dn.fsample['force'])

            if et_samples > 0: #dat_tmp.iloc[ntrial].trialPoint == 1:
                rt = dat_row['RT'][0] / 1000
                rt_samples = int(rt * dn.fsample['force'])
                trialPoint = 1
            else:
                rt_samples = 0
                trialPoint = 0
                et_samples = len(force_tmp) - int(dn.hold_time * dn.fsample['force'])

            print(f'subj{sn}, session {session}, block {bl}, ntrial {ntrial + 1}, et {et_samples}, rt {rt_samples}, '
                  f'trialPoint {trialPoint}')
            ...
            single_trial_metrics['thumb_der_peak'].append(force_der1_peak[0])
            single_trial_metrics['index_der_peak'].append(force_der1_peak[1])
            single_trial_metrics['middle_der_peak'].append(force_der1_peak[2])
            single_trial_metrics['ring_der_peak'].append(force_der1_peak[3])
            single_trial_metrics['pinkie_der_peak'].append(force_der1_peak[4])
            single_trial_metrics['thumb_der_t2peak'].append(force_der1_t2p_samples[0]/ dn.fsample['force'])
            single_trial_metrics['index_der_t2peak'].append(force_der1_t2p_samples[1]/ dn.fsample['force'])
            single_trial_metrics['middle_der_t2peak'].append(force_der1_t2p_samples[2]/ dn.fsample['force'])
            single_trial_metrics['ring_der_t2peak'].append(force_der1_t2p_samples[3]/ dn.fsample['force'])
            single_trial_metrics['pinkie_der_t2peak'].append(force_der1_t2p_samples[4]/ dn.fsample['force'])
            single_trial_metrics['session'].append(dat_row['day'][0])
            single_trial_metrics['week'].append(dat_row['week'][0])
            single_trial_metrics['session_type'].append(dat_row['session'][0])
            single_trial_metrics['RT'].append(dat_row['RT'][0])
            single_trial_metrics['ET'].append(et_samples / dn.fsample['force'])
...
def main(args):
    if args.what == 'single_trial':
        single_trial_metrics = calc_single_trial_metrics(args.sn, args.day, args.experiment_code)
        single_trial_metrics.to_csv(os.path.join(gl.baseDir, 'behavioural', f'day{args.day}',
    ...
    if args.what == 'pool_behaviour':
        ...
        for sn in args.sns:
            for day in range(24):
                print(f'doing participant {sn}, day {day + 1}')
                data = pd.read_csv(os.path.join(gl.baseDir, gl.behavDir, f'day{day+1}',
                             f'{args.experiment_code}_{sn}_single_trial.tsv'), sep = '\t',)
                data_pooled = pd.concat([data_pooled, data])
        data_pooled.to_csv(os.path.join(gl.baseDir, gl.behavDir, f'single_trial_behaviour.tsv'), sep = '\t', index=False)

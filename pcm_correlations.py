...
def main(args):
    if args.what == 'correlation_across_sessions':
            ...
            for chord in chords:
                for H in Hem:
                    for roi in rois:
                        N = len(args.sns)
                        Y = list()
                        print(f'doing...ROI.{H}.{roi}, sess {int(corr[0])} vs. {int(corr[1])}, {chord} chords')
                        ...
                        np.save(os.path.join(pcm_path, f'G_obs.corr_across_sess.glm{args.glm}.{H}.{roi}.npy'), G_obs)
        ...
        df.to_csv(os.path.join(pcm_path, 'correlation_across_sessions.BOLD.tsv'), sep='\t', index=False)

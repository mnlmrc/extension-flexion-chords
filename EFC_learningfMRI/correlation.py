...
def correlation_sess(sns, glm, rois, atlas_name='ROI'):
                ...
                for H in gl.Hem:
                    for roi in rois:
                        ...
                        for s, sn in enumerate(sns):
                            ...
                            print(f'doing...participant {sn}, {H}, {roi}, sess {int(corr[0][-2:])} vs. {int(corr[1][-2:])}, {chord} chords')
                        ...
                        r_indiv, r_group, SNR, _, _, _ = calc_mle_corr(Mflex, theta[0], theta_gr[0], cond_effect=True)
                        df_tmp = pd.DataFrame()

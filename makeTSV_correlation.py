    ...
    for glm in [1,]:
        ...
        for H in im.Hem:
            for roi in rois:
                for corr in corrs:
                    for chord in chords:
                        cov = np.load(os.path.join(pth.baseDir, pth.pcmDir, f'G_obs.corr_across_sess.glm{glm}.{corr}.{chord}.{H}.{roi}.npy'))
        ...
        r_xval.to_csv(os.path.join(pth.baseDir, pth.pcmDir, f'xval_correlation.{atlas}.glm{glm}.tsv'), sep='\t', index=False)
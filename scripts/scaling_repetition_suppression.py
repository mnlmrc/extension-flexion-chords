...
if __name__=='__main__':
    ...
    for session, sn, H, roi in itertools.product(gl.sessions, sns, gl.Hem, rois):
        ...
        G_ref = np.load(os.path.join(gl.baseDir, gl.pcmDir, f'subj{sn}', f'G_obs.within_session.{session}.1.glm{glm}.{H}.{roi}.npy'))
        G_tar = np.load(os.path.join(gl.baseDir, gl.pcmDir, f'subj{sn}', f'G_obs.within_session.{session}.2.glm{glm}.{H}.{roi}.npy'))
    ...
    scaling.to_csv(os.path.join(gl.baseDir, gl.pcmDir, f'scaling.repetition_suppression.glm{glm}.{atlas_name}.tsv'), index=False, sep='\t')
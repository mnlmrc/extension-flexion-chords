...
if __name__=='__main__':
    ...
    for session, sn, H, roi in itertools.product(gl.sessions, sns, gl.Hem, rois):
        ...
        G_ref = np.load(os.path.join(gl.baseDir, gl.pcmDir, f'subj{sn}', f'G_obs_raw.within_session.3.glm{glm}.{H}.{roi}.npy'))
        G_tar = np.load(os.path.join(gl.baseDir, gl.pcmDir, f'subj{sn}', f'G_obs_raw.within_session.{session}.glm{glm}.{H}.{roi}.npy'))

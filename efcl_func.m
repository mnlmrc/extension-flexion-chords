function varargout = efcl_func(what, varargin)
    ...
    switch(what)
        case 'BIDS:move_unzip_raw_func'
            ...
            for run = runs
                ...
                func_raw_path = fullfile(baseDir,bidsDir,day_id,sprintf('subj%.02d',sn),  'func',FuncRawName_tmp);
        
                % destination path:
                output_folder = fullfile(baseDir,imagingRawDir,subj_id);
                output_file = fullfile(output_folder,[subj_id sprintf('_run_%.02d.nii.gz',run + 10 * d)]);
        ...
        case 'BIDS:move_unzip_raw_sbref'
            % Moves, unzips and renames raw functional (BOLD) images from 
            % BIDS directory. After you run this function you will find
            % nRuns Nifti files named <subj_id>_run_XX.nii in the 
            % <project_id>/imaging_data_raw/<subj_id>/ directory.
            ...
            for run = runs
                ...
                % sbref name:
                SBrefName = sprintf('sub-%d_task-task_run-%.02d_part-mag_sbref.nii.gz', sn, run);

                % path to the subj func data:
                func_raw_path = fullfile(baseDir,bidsDir,day_id,sprintf('subj%.02d',sn),  'func', SBrefName);
        
                % destination path:
                output_folder = fullfile(baseDir,imagingRawDir,subj_id);
                output_file = fullfile(output_folder,[subj_id sprintf('_sbref_run_%.02d.nii.gz',run + 10 * d)]);
        ...
        case 'BIDS:move_unzip_raw_fmap'
            ...
            magnitude_path = fullfile(baseDir,bidsDir,day_id, sprintf('subj%.02d',sn),'fmap',magnitude);
            phase_path = fullfile(baseDir,bidsDir,day_id,sprintf('subj%.02d',sn),'fmap',phase);
    
            % destination path:
            output_folder = fullfile(baseDir,fmapDir,subj_id );
            output_magnitude = fullfile(output_folder,[day_id '_' subj_id '_magnitude.nii.gz']);
            output_phase = fullfile(output_folder,[day_id '_' subj_id '_phase.nii.gz']);
        ...
        case 'FUNC:make_fmap' 
            ...
            [et1, et2, tert] = spmj_et1_et2_tert(fullfile(baseDir, bidsDir, day_id,subj_id,  'fmap'),...
                                                fullfile(baseDir, bidsDir, day_id,subj_id,  'func'),...
                                                num2str(subj_row.sn));

            spmj_makefieldmap(fullfile(baseDir, fmapDir, subj_id ), ...
                              sprintf('%s_%s_magnitude.nii', day_id,subj_id),...
                              sprintf('%s_%s_phase.nii', day_id,subj_id),...
          ...
          for run=1:numel(runs)
              oldName = sprintf('vdm5_sc%s_%s_phase_run_%d.nii',day_id, subj_id, run);
              newName = sprintf('vdm5_sc%s_phase_run_%d.nii', subj_id, run + 10 * d);
              movefile(fullfile(baseDir, fmapDir, subj_id, oldName), fullfile(baseDir, fmapDir, subj_id, newName))
          end
        ...
        case 'FUNC:realign'      
            ...
            run_list = {}; % Initialize as an empty cell array
            for run = runs
                path = fullfile(baseDir,imagingRawDir,subj_id,sprintf('%s_run_%02d.nii', subj_id, run));
                V = spm_vol(path);
                nvol = size(V, 1);
                for vol =1:nvol
                    run_list{end+1} = [run_list, spm_read_vols(V(nvol))];
                end
            end

            spmj_realign(run_list);
...
%             vararginoptions(varargin,{'sn','prefix','rtm'})
        ...
        case 'FUNC:coreg_sbref'                                                      
            ...
            mean_file_name = sprintf('%s_sbref_run_%.02d.nii', subj_id, run);
            J.source = {fullfile(baseDir,imagingRawDir,subj_id, mean_file_name)}; 
            J.ref = {fullfile(baseDir,anatomicalDir,subj_id,[subj_id, '_T1w','.nii'])};
            J.other = {''};
            J.eoptions.cost_fun = 'nmi';
            J.eoptions.sep = [4 2];
...
%             vararginoptions(varargin,{'sn','prefix','rtm'})
            ...
            for r = 1:length(run_list)
                for i = 1:str2double(pinfo.numTR{1})
                     Q{end+1} = fullfile(baseDir,imagingDir,subj_id,sprintf('%s%s_%s.nii,%d', prefix, subj_id, run_list{r}, i));
                end
            end
